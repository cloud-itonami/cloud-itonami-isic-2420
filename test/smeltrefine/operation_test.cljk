(ns smeltrefine.operation-test
  "Smoke tests for the compiled SmeltRefineOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `smeltrefine.governor-contract-test`; the Store contract in
  `smeltrefine.store-contract-test`."
...
(deftest test-actor-builds
  (testing "SmeltRefineOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "furnace-001" :severity :moderate :description "toxic fume rise near smelting furnace"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
