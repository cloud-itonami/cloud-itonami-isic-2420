(ns smeltrefine.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the smelting furnace, refining
  furnace, electrolytic refining cell, or converter directly... does
  NOT authorize or execute maintenance/furnace operations') implemented
  faithfully. The single invariant under test:

    SmeltRefineAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Precious & Non-Ferrous
    Metals Smelting-Refining Plant Operations Governor would reject;
    `:schedule-maintenance`/`:flag-safety-concern`/`:coordinate-
    shipment` NEVER auto-commit at any phase; `:log-production-batch`
    (no physical/financial risk) MAY auto-commit when clean; and every
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:purity-grade :gold-9999-fine}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :gold-9999-fine (:purity-grade (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-smelting-furnace :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest invalid-purity-grade-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:purity-grade :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-purity-grade} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:purity-grade (store/batch db "batch-001"))) "fabricated purity-grade never lands in the SSoT")))

(deftest invalid-impurity-rate-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:impurity-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-impurity-rate} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:impurity-percent (store/batch db "batch-001"))) "fabricated impurity-rate never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "furnace-001" :severity :moderate
                                            :description "molten-metal splash risk near smelting furnace"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:purity-grade :fabricated-grade}} coordinator)
