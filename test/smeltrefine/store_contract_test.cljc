(ns smeltrefine.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `smeltrefine.store` ns docstring for why a second (Datomic-
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (false? (:verified? (store/equipment-unit s "refine-002"))))
    (is (false? (:registered? (store/equipment-unit s "refine-002"))))
    (is (= ["furnace-001" "refine-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:purity-grade :gold-999-fine}})
    (is (= :gold-999-fine (:purity-grade (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "refine-002" :severity :high}})
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-weight
  (let [s (seeded)]
    ...
    (is (= 7.5 (:shipped-weight-kg (store/batch s "batch-001")))
        "2.5 seeded + 5.0 committed")))
