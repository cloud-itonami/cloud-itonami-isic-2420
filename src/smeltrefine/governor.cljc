(ns smeltrefine.governor
  "Precious & Non-Ferrous Metals Smelting-Refining Plant Operations
  Governor -- the independent compliance layer that earns the
  SmeltRefineAdvisor the right to commit. The advisor has no notion of
  whether a piece of equipment it wants to schedule maintenance
  against has actually been inspected/registered, whether a batch it
  wants to coordinate a shipment against has actually been
  assay-verified/registered, whether a maintenance proposal secretly
  tries to ACTUATE (rather than merely draft-schedule) the smelting
  furnace, refining furnace, electrolytic refining cell, or converter,
  whether a shipment proposal's own claimed weight would blow through
  the batch's own logged production weight, or when an act stops being
  a coordination proposal and becomes direct smelting/refining-
  furnace-equipment control, so this MUST be a separate system able to
  *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:precious-nonferrous-smelting-refining-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       or `:refining-cell/energize`) is
                                       the 'direct smelting/refining-
                                       furnace-equipment control' scope
    ...
    4. Furnace-actuate blocked     -- for `:schedule-maintenance`, does
                                       ...
                                       smelting furnace, refining
                                       furnace, electrolytic refining
                                       cell, or converter is this
    ...
    9. Invalid purity-grade        -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:purity-grade` outside the
                                       closed known set
                                       (`smeltrefine.registry/purity-
                                       grade-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated purity grade through.
   10. Invalid impurity rate       -- for `:log-production-batch`, if
                                       the patch declares an
                                       `:impurity-percent` that is not
                                       a physically plausible reading
                                       (`smeltrefine.registry/
                                       impurity-valid?`), the batch
                                       record is rejected rather than
                                       let fabricated/assay-error data
                                       through.
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [smeltrefine.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct smelting/
  refining-furnace-equipment-control effect."
...
(defn- furnace-control-blocked-violations
  ...
  Anything else (direct smelting-furnace/refining-furnace/electrolytic-
  refining-cell/converter control, a fabricated actuation effect) is
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :furnace-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は溶解炉・精錬炉・電解精錬セル・コンバータ等の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- furnace-actuate-blocked-violations
  ...
  to directly actuate the smelting furnace, refining furnace,
  electrolytic refining cell, or converter -- this actor may only ever
  propose/schedule a DRAFT maintenance window, never actuate equipment
  directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-furnace? (:value proposal))))
    [{:rule :furnace-actuate-blocked
      :detail "溶解炉・精錬炉・電解精錬セル・コンバータの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-purity-grade-violations
  "For `:log-production-batch`, if the patch declares a
  `:purity-grade` outside the closed known set, reject rather than let
  a fabricated purity grade through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [purity-grade (:purity-grade (:value proposal))]
      (when (and (some? purity-grade) (not (registry/purity-grade-valid? purity-grade)))
        [{:rule :invalid-purity-grade
          :detail (str purity-grade " は既知の purity-grade 値ではない")}]))))

(defn- invalid-impurity-violations
  "For `:log-production-batch`, if the patch declares an
  `:impurity-percent` that is not a physically plausible reading,
  reject rather than let fabricated/assay-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rr (:impurity-percent (:value proposal))]
      (when (and (some? rr) (not (registry/impurity-valid? rr)))
        [{:rule :invalid-impurity-rate
          :detail (str rr "% は物理的に妥当な不純物率の範囲外")}]))))

(defn check
  "Censors a SmeltRefineAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-purity-grade-violations request proposal)
                           (invalid-impurity-violations request proposal)))
