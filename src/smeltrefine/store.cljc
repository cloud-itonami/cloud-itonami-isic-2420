(ns smeltrefine.store
  "SSoT for the precious & non-ferrous metals smelting-refining plant-
  ...
  (`cloud-itonami-isic-2432`'s own `nonferrousmfg.store`), this build
  ...
  self-contained (no external smelting-refining capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             smelted/refined doré/bullion/cathode/
                             anode/ingot lot)'s purity-grade/weight/
                             impurity record. `:verified?` marks
                             whether the batch's own claims have
                             actually been assayed (never inferred
    ...
    - `equipment`         -- a smelting-furnace/refining-furnace/
    ...
    - `maintenance`       -- a scheduled smelting/refining-equipment
                             maintenance-window DRAFT against a piece
                             of equipment (`smeltrefine.registry`'s
    ...
    - `shipments`         -- a proposed outbound refined-metal
                             shipment DRAFT (`smeltrefine.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (smeltrefine.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (smeltrefine.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :purity-grade :gold-9999-fine :output-form :bullion-bar
                :material "Gold Bullion Bar, LBMA Good Delivery Refined 9999 Fine"
                :weight-kg 12.5 :impurity-percent 0.01
                :verified? true :registered? true
                :shipped-weight-kg 2.5
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :purity-grade :copper-cathode-grade-a :output-form :cathode
                :material "Copper Cathode, LME Grade A Electrolytically Refined"
                :weight-kg 3000.0 :impurity-percent 0.02
                :verified? true :registered? true
                :shipped-weight-kg 2800.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :purity-grade :aluminum-p1020 :output-form :ingot
                :material "Aluminum P1020 Primary Ingot"
                :weight-kg 12000.0 :impurity-percent 2.5
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"furnace-001" {:id "furnace-001" :kind :flash-smelting-furnace
                  :verified? true :registered? true
                  :last-maintenance-date "2026-05-01"}
   "refine-002" {:id "refine-002" :kind :electrolytic-refining-cell
                 :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `smeltrefine.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `smeltrefine.registry` and
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered flash-smelting-
  furnace unit (schedulable for maintenance), one UNVERIFIED/
  unregistered electrolytic-refining-cell unit (blocks any maintenance
