(ns smeltrefine.operation
  "SmeltRefineOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (SmeltRefineAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Precious &
  Non-Ferrous Metals Smelting-Refining Plant Operations Governor
  ...
  (:require [langgraph.graph :as g]
            ...
            [smeltrefine.advisor :as advisor]
            [smeltrefine.governor :as governor]
            [smeltrefine.phase :as phase]
...
(defn build
  "Compiles a SmeltRefineOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `smeltrefine.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SmeltRefineAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Precious & Non-Ferrous Metals Smelting-Refining Plant Operations Governor -- independent censor.
      (g/add-node :govern
