(ns smeltrefine.advisor
  "SmeltRefineAdvisor -- the *contained intelligence node* for the
  precious & non-ferrous metals smelting-refining plant-operations
  coordination actor.

  It normalizes production-batch patches (purity-grade/weight/
  impurity), drafts a smelting-furnace/refining-furnace/electrolytic-
  refining-cell/converter maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound refined precious/non-ferrous metal (gold/silver/copper/
  aluminum) shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real smelting/refining-furnace-
  equipment actuation or freight dispatch. Every output is censored
  downstream by `smeltrefine.governor` before anything touches the
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `smeltrefine.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct smelting/
                                 ; refining-furnace-control effect
  ...
  `smeltrefine.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [smeltrefine.registry :as registry]
            [smeltrefine.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch (smelted/refined lot) intake upsert -- the advisor
  only normalizes/validates the patch; it does not invent the batch's
  purity-grade, weight, impurity or verification status. High
...
(defn- schedule-maintenance
  "Draft a smelting-furnace/refining-furnace/electrolytic-refining-
  cell/converter maintenance-window scheduling proposal against a
  piece of equipment. The advisor reports what it can see (equipment
  verified?/registered?) in its rationale, but `smeltrefine.governor`
  NEVER trusts this report -- it independently re-derives verified?/
  registered? from the equipment's own stored fields before any commit
  is possible."
...
(defn- flag-safety-concern
  "Draft a molten-metal-hazard (splash/burn, furnace/converter off-gas
  exposure, toxic heavy-metal-fume exposure, electrolytic-refining-
  cell acid-mist exposure)/environmental concern (stack emissions,
  effluent, tailings/slag release). ALWAYS `:stake
  ...
  `smeltrefine.phase`: no phase ever adds this op to a phase's `:auto`
  set; `smeltrefine.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound refined precious/non-ferrous metal (gold/silver/
  copper/aluminum) shipment coordination proposal against a production
  batch. The advisor passes through the caller's own claimed weight --
  it does NOT invent one, and `smeltrefine.governor` NEVER trusts it:
  it independently recomputes whether the batch's own cumulative-
  shipped weight plus this claim would exceed the batch's own recorded
  weight before any commit is possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたは貴金属・非鉄金属(金・銀・銅・アルミニウム)製錬・精錬プラント"
       "運用コーディネーターの助言者です。"
       ...
       "溶解炉・精錬炉・電解精錬セル・コンバータ等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `smeltrefine.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :smeltrefine-advisor-proposal
   :op         (:op request)
